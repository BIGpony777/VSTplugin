...
tresult PLUGIN_API SineSynthProcessor::process (Vst::ProcessData& data)
{
		...
		for (int32 i = 0; i < numEvent; i++) {
			...
			if (events->getEvent(i, event) == kResultOk) {
				...
				case Vst::Event::kNoteOnEvent:
					...
					fDeltaAngle = P12 * fFrequency / data.processContext->sampleRate;
					fVolume = 0.3f;
					fOsc1Phase = 0.f;
					fOsc2Phase = 0.f;
					fOsc3Phase = 0.f;
	...
	for (int32 i = 0; i < data.numSamples; i++) {
		...
		outL[i] *= fVolume * (fSlide1 * 10);
		outR[i] = outL[i];

		fOsc1Phase += fDeltaAngle * (1.f * fPit1);
		fOsc2Phase += fDeltaAngle * (1.f * fPit2);
		fOsc3Phase += fDeltaAngle * (1.f * fPit3);
	}
	return kResultOk;
}
...
tresult PLUGIN_API SineSynthProcessor::getState (IBStream* state)
{
	...
	streamer.writeFloat(fSlide1);
	streamer.writeFloat(fPit1);
	streamer.writeFloat(fPit2);
	streamer.writeFloat(fPit3);
	return kResultOk;
}
